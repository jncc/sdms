...
randomOcc <- function(presframe, #presence data frame
                      ...
                      lowestResm = 10000, #lowest resolution to limit
                      covarResm = 1000) {  # resolution of covariants used
  ...
  if(!is.numeric(presframe[[precisionCol]]) ) {
    ...
    presframe[[precisionCol]] <- ifelse(presframe$res == "m", presframe[[precisionCol]], presframe[[precisionCol]]*1000)
    presframe$res <- NULL
  }
  ...
  for (j in unique(sort(presframe[[precisionCol]], decreasing = TRUE))) { # Run per capture resolution grid size
    if( j %in% presframe[[precisionCol]] & lowestResm >= j) { #If capture resolution is within lowest resolution to use limit
      ...
      if(j > covarResm){
        ...
        df$easting <- df$easting + round(stats::runif(length(df$easting), -movemax, movemax)) #Move east/west by up to half capture grid cell size
        df$northing <- df$northing + round(stats::runif(length(df$northing), -movemax, movemax)) #Move north/south by up to half capture grid cell size
