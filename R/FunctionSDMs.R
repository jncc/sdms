...
SDMs <- function (occ = occurrence, #occurence data, spatialpointsdataframe of presence points
  ...
  #bgNonPres <- raster::mask(bckg, occ, inverse=TRUE)
  #bg.pts <- raster::sampleRandom(bgNonPres, max_bg_points, xy = TRUE, sp=TRUE, na.rm = TRUE) ## Due to the small depth range of the mask, this function returned dramatically fewer points than max_bg_points (~10%). I've therefore substituted for randomPoints from the Dismo package below, and given it up to 50 attempts to generate the number of points requested. Also moved this to after the filtering of pres_vars for complete.cases, so that the number of background points can match the number of presence points.
  ...
  if ("MaxEnt" %in% models) {
    Maxent <- tryCatch({dismo::maxent(vars_drop, presence_train[,c("x","y")],
                               args = c('nolinear', 'noquadratic', 'noproduct', 'nothreshold'), path = "/out")
    }, error=function(err) NA)
    eval_test <- tryCatch(dismo::evaluate(presence_test[,c("x","y")], bg_test[,c("x","y")], Maxent, vars_drop),
  ...
  if ("BioClim" %in% models) {
    BioClim <- tryCatch(dismo::bioclim(vars_drop, presence_train[,c("x","y")]), error=function(err) NA)
    eval_test <- tryCatch(dismo::evaluate(presence_test[,c("x","y")], bg_test[,c("x","y")], BioClim, vars_drop),
  ...
  if ("SVM" %in% models) {
    SVM <- tryCatch(kernlab::ksvm(stats::as.formula(paste("Presence ~", paste(varfactors,collapse = "+") )), data = combo_train[,3:ncol(combo_train)], na.action=stats::na.omit, kpar="automatic"), error=function(err) NA)
    eval_test<- tryCatch(dismo::evaluate(presence_test[,3:ncol(presence_test)], bg_test[,3:ncol(bg_test)], SVM),
  ...
  if ("RF" %in% models) {
    mtry <- tryCatch(randomForest::tuneRF(x = combo_train[,3:(ncol(combo_train)-1)], y = combo_train[,ncol(combo_train)],trace=FALSE,plot=FALSE), error=function(err) NA)
    if(!is.na(mtry)) {
      best.m <- tryCatch(mtry[mtry[,2]==min(mtry[, 2]),1], error=function(err) NA)
      RF <- tryCatch(randomForest::randomForest(stats::as.formula(paste("Presence ~", paste(varfactors,collapse = "+") )), data = combo_train[,3:ncol(combo_train)], na.action=stats::na.omit, mtry=best.m), error=function(err) NA)
    } else {
      RF <- tryCatch(randomForest::randomForest(stats::as.formula(paste("Presence ~", paste(varfactors,collapse = "+") )), data = combo_train[,3:ncol(combo_train)], na.action=stats::na.omit), error=function(err) NA)
    }
    eval_test <- tryCatch(dismo::evaluate(presence_test[,3:ncol(presence_test)], bg_test[,3:ncol(bg_test)], RF),
  ...
  if ("GLM" %in% models) {
    glm_multi <- tryCatch(do.call("glmulti", list(y = "Presence", xr = varfactors, family="binomial", data=combo_train[,3:ncol(combo_train)], na.action=stats::na.omit, level=1, method="g", confsetsize=1, deltaB=0, conseq=1, maxit=200, report=FALSE, plotty=FALSE)), error=function(err) NA)
    if (class(glm_multi) == "numeric") {
      GLM <- tryCatch(stats::glm(stats::as.formula(paste("Presence ~", paste(varfactors,collapse = "+") )), family="binomial", data=combo_train[,3:ncol(combo_train)], na.action=stats::na.omit), error=function(err) NA)
    } else {
      ...
      GLM <- tryCatch(stats::glm(gl_string, family="binomial", data=combo_train[,3:ncol(combo_train)], na.action=stats::na.omit), error=function(err) NA)
    }
    eval_test <- tryCatch(dismo::evaluate(presence_test[,3:ncol(presence_test)], bg_test[,3:ncol(bg_test)], GLM),
  ...
  #GAM
  if ("GAM" %in% models) {
    GAM <- tryCatch(mgcv::gam(stats::as.formula(paste("Presence ~ s(", paste(varfactors,collapse = ")+ s("),")" )), data=combo_train[,3:ncol(combo_train)], family=stats::binomial(link = "logit"), gamma=1.4, na.rm = TRUE), error=function(err) NA)
    eval_test <- tryCatch(dismo::evaluate(presence_test[,3:ncol(presence_test)], bg_test[,3:ncol(bg_test)], GAM),
  ...
  if ("BRT" %in% models) {
    ...
    BRT <- tryCatch(dismo::gbm.step(data = combo_train, gbm.x = 3:(ncol(combo_train)-1), gbm.y = as.numeric(ncol(combo_train)), tree.complexity = 3, learning.rate = 0.01, bag.fraction = 0.75, verbose = FALSE, silent = TRUE, plot.main = FALSE), error=function(err) NA)
    eval_test <- tryCatch(dismo::evaluate(presence_test[,3:ncol(presence_test)], bg_test[,3:ncol(bg_test)], BRT, n.trees=BRT$n.trees),
  ...
  Obs2ras <- function(points, raskm = covarReskm) {
    # nr <- 1250 / raskm
    # nc <- 700 / raskm
    # r <- raster::raster(nrows = nr, ncols = nc, xmn = 0, xmx= 700000, ymn = 0, ymx = 1250000)
    r <- vars[[1]]
    r[!is.na(r[])] <- 0
  ...
  prediction_best <- tryCatch(kernlab::predict(vars_drop, bestmod, type="response"), error=function(err) Obs2ras(ppts[c("x","y")]))
...
all_evals <- cbind(data.frame(Models), data.frame(matrix(unlist(all_evals), nrow=length(models)+1, byrow=F), stringsAsFactors=FALSE))
...
raster::writeRaster(Mean_predict, file = paste("Outputs/", lab, tries, sep=""), format="GTiff")
